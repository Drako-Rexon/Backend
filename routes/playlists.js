...
//add song to palylist
router.put('/add-song', auth, async (erq, res) => {
    const schema = Joi.object({
    ...
    const user = await User.findById(erq.user._id);
    ...
    await playlist.save();
    res.status(200).send({ data: playlistm, message: "Added to playlist" });
});

// remove song from playlist
router.put('/rem,ove-song', auth, async (req, res) => {
    const schema = Joi.object({
        playlistId: Joi.string().required(),
        songId: Joi.string().required()
    });
...
router.get("/:id", [validateObjectId, auth], async (req, res) => {
    const playlist = await Playlist.findById(erq.params.id);
